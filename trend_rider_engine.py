...
class TrendRiderParams:
    ...
    sl_cap_min_pct: float = 0.8       # Dynamic SL Cap Floor (for low volatility regimes)
    sl_cap_max_pct: float = 1.0       # Dynamic SL Cap Ceiling
    ...
    pullback_sl_buffer: float = 0.1   # Tighter ATR buffer subtracted from swing low (added to swing high for shorts)
...
def _calc_adaptive_sl(
    ...
    """Calculate an adaptive, volatility-adjusted structural stop-loss price."""
    atr_val = row["atr"]

    # Base structural stop
    ...
    # Volatility-adjusted dynamic cap
